(function () {
    ...
    questionnaire.innerHTML = allSections.map(function (section) {
      ...
      const questions = section.rows.map(function (row) {
        ...
        return "<article class=\"question\" data-row=\"" + row.id + "\">" +
          ...
          "<label class=\"item-note\">Observaciones<textarea id=\"note-" + row.id + "\" rows=\"2\"></textarea></label></div></article>";
      }).join("");
  ...
  function noteFor(row) {
    return document.getElementById("note-" + row.id).value.trim();
  }
    ...
    allRows.forEach(function (row) {
      ...
      if (isAsymmetric(row) || noteFor(row)) {
        addPreviewMarker(layer, noteFor(row) ? "preview-note" : "preview-asym", {
          ...
          top: row.top,
          height: row.height
        }, (isAsymmetric(row) ? "A" : "") + (isAsymmetric(row) && noteFor(row) ? " / " : "") + (noteFor(row) ? "Obs." : ""));
      }
    });
  }
  ...
  function drawAnswer(page, row, option, regular, bold, rgb) {
    ...
    if (noteFor(row)) {
      drawTextTop(page, "Obs.", commentX + 2, row.top + (isAsymmetric(row) ? 12 : 3), 5.5, bold, rgb(0.07, 0.25, 0.34));
    }
  }
